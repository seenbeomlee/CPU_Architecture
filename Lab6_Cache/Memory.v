...
module Memory(clk, reset_n, readM1, address1, data1, readM2, writeM2, address2, data2, is_ready);
   ...
   */
   ...
   assign I_hit = (address1[15:4] == cache[address1[3:2]][77:66]) && cache[address1[3:2]][`VALID];
   assign D_hit = (address2[15:4] == cache[address2[3:2]][77:66]) && cache[address2[3:2]][`VALID];

   always@(posedge clk) begin

      $display("I_hit: %d, D_hit: %d, readM1: %d, readM2: %d, writeM2: %d /// address1: %d, address2: %d", I_hit, D_hit, readM1, readM2, writeM2, address1, address2);
      ...
      if(!reset_n)
         ...
         if(I_hit && (D_hit || !(readM2 || writeM2))) begin
               ...
               if(readM2) begin // CPU requires data from memory
                  case(address2[1:0])// find the proper cache line and return the outputData2(=data)
               ...
               if(writeM2) begin // CPU wants to write data to memory
                  ...
                  endcase
                  cache[address2[3:2]][`DIRTY] <= 1'b1;// we need to indicate Dirty bit for later update in memory
               end
               is_ready <= 1'b1;
         end
            ...
            if(is_ready) begin /* cache miss */
               $display("qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq");
               ...
               if(readM1) begin
                  if(writeM2 & address1==address2) begin
                     ...
                     case(address1[1:0])
                        2'b00 : data1 <= memory[{address1[`WORD_SIZE-1:2],2'b00}];
                        2'b01 : data1 <= memory[{address1[`WORD_SIZE-1:2],2'b01}];
                        2'b10 : data1 <= memory[{address1[`WORD_SIZE-1:2],2'b10}];
                        2'b11 : data1 <= memory[{address1[`WORD_SIZE-1:2],2'b11}];
                     endcase
                  end
               end
               ...
               if(readM2) begin 
                  case(address2[1:0])
                     2'b00 : outputData2 <= memory[{address2[`WORD_SIZE-1:2],2'b00}];
                     2'b01 : outputData2 <= memory[{address2[`WORD_SIZE-1:2],2'b01}];
                     2'b10 : outputData2 <= memory[{address2[`WORD_SIZE-1:2],2'b10}];
                     2'b11 : outputData2 <= memory[{address2[`WORD_SIZE-1:2],2'b11}];
                  endcase
               end

               if(writeM2) begin
                  if(cache[address2[3:2]][`DIRTY]) begin
                     $display("qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq");
                     memory[{cache[address2[3:2]][77:66],address2[3:2],2'b00}] <= cache[address2[3:2]][15:0];
                     memory[{cache[address2[3:2]][77:66],address2[3:2],2'b01}] <= cache[address2[3:2]][31:16];
                     memory[{cache[address2[3:2]][77:66],address2[3:2],2'b10}] <= cache[address2[3:2]][47:32];
                     memory[{cache[address2[3:2]][77:66],address2[3:2],2'b11}] <= cache[address2[3:2]][63:48];
                  end
               end
